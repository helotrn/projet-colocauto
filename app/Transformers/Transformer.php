...
class Transformer
{
    ...
    {
        foreach (
            array_merge($item->collections, array_keys($item->morphManys))
            as $relation
        ) {
            ...
            if ($this->shouldIncludeRelation($relation, $item, $options)) {
                ...
                $output[$relation] = $target->map(function ($p) use (
                    $transformer,
                    $options,
                    $relation
                ) {
    ...
    {
        return $target->map(function ($p) use (
            $relation,
            $transformer,
            $options
        ) {
