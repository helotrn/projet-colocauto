...
class ActionController extends RestController
{
    ...
    public function create(CreateRequest $request)
    {
        switch ($request->get("type")) {
            case "extension":
                return $this->extensionController->create(
    ...
    public function update(ActionRequest $request, $id)
    {
        switch ($request->get("type")) {
            case "intention":
                ...
                return $this->intentionController->update(
                    $intentionRequest,
                    $id
                );
            case "payment":
            ...
            case "extension":
                ...
                return $this->extensionController->update(
                    $extensionRequest,
                    $id
                );
            case "handover":
    ...
    public function cancel(ActionRequest $request, $loanId, $actionId)
    {
        switch ($request->get("type")) {
            case "intention":
                return $this->intentionController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
            case "pre_payment":
                return $this->prePaymentController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
            case "payment":
                return $this->paymentController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
            case "takeover":
                return $this->takeoverController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
            case "extension":
                return $this->extensionController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
            case "handover":
                return $this->handoverController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
            case "incident":
                return $this->incidentController->cancel(
                    $request,
                    $actionId,
                    $loanId
                );
